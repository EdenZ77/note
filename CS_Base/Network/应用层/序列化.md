...
在我提供的示例中，只展示了 `Person` 结构体的定义以及一些接口方法，并没有包括序列化（`Marshal`）和反序列化（`Unmarshal`）的具体逻辑。为了实现序列化和反序列化，`protoc-gen-go` 插件会生成一些辅助函数和方法，这些方法会调用 protobuf 运行时库中的通用逻辑来执行实际的序列化和反序列化过程。

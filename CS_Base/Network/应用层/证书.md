...
<img src="image/image-20241031161010792.png" alt="image-20241031161010792" style="zoom:67%;" />
...
假设你有一个证书文件 `server.crt` 和一个 CA 证书文件 `ca.crt`，你想验证 `server.crt` 是否由 `ca.crt` 签名。

```shell
openssl x509 -noout -text -in server.crt -CA ca.crt
# -noout：不输出原始证书数据。
# -text：以可读文本格式显示证书内容。
# -in server.crt：指定输入的证书文件。
# -CA ca.crt：指定用于验证的 CA 证书文件。
```
...
假设你有一个证书文件 `server.crt`，你想将其转换为 PKCS#7 格式。

```shell
openssl x509 -in server.crt -outform pem -out server.p7b -CAfile ca.crt
# -in server.crt：指定输入的证书文件。
# -outform pem：指定输出格式为 PEM（PKCS#7 格式通常也是 PEM 编码）。
# -out server.p7b：指定输出文件。
# -CAfile ca.crt：指定包含 CA 证书的文件。
```

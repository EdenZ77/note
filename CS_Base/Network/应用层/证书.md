...
5. Signature ：证书签名，用当前证书的 CA 私钥对当前证书的主体信息进行签名。可以用当前证书的 CA 公钥对当前证书的 Signature 进行验签。
...
特别注意，每一级证书私钥并不是放在证书里面的，而是有一个单独的私钥文件，当前这一级的私钥用于签发下一级证书，不对外公开。私钥应该由当前的 CA 严格保管，不能外泄！！！

...
3. `epoll_wait`：等待事件发生
   这是核心的等待调用。它检查 eventpoll 的就绪队列是否为空。当 fd 就绪时，内核通过之前注册的回调函数将其放入就绪队列。`epoll_wait`返回时，只将就绪队列中的事件拷贝给用户。应用程序无需遍历所有 fd，直接处理返回的事件数组即可。
...
1. 无性能瓶颈：`epoll`不会因为 fd 数量的增加而导致性能显著下降。epoll_wait的时间复杂度是O(k)，其中k是就绪的fd数量，而不是O(n)。

...
- [ ] pprof 
  - [ ] 通过Prometheus告警规则触发，发现大量协程和内存偏高
  - [ ] 然后使用pprof的命令行查看实时状态和火焰图观察
  - [ ] 协程泄露：1、为每个租户的每个安全配置都开协程进行处理，导致大量协程（使用协程池ants限制总数）；2、channel阻塞导致的协程泄露（增加default或者超时机制）。
  - [ ] 内存：每次处理都创建新对象（buffer_pool）、频繁序列化/反序列化（直接赋值处理）

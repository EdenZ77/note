...
Kubernetes 是基于 API 的基础设施， Kubernetes 中的概念都被抽象成各种资源（ Resource），不同的资源（如我们熟悉并经常使用的 Deployment 资源、 Pod 资源、 StatefulSet 资源、 Service 资源、 ConfigMap 资源、 Node 资源等）拥有不同的功能。在 Kubernetes 的世界里对各种资源的操作都是基于 API 来完成的， Kubernetes 通过 kube-apiserver 提供一系列的 REST API 来完成对资源的基本操作。  
...
`kubectl api-resources` 命令实际上列出的不是完整的 API 资源类型。如 `deployments/scale`子资源就没有在结果中被列出。要想列出完整的 API 资源类型，需要先执行 `kubectl get --raw /` 命令，然后根据返回的所有 Path 递归查询所有 API 组支持的资源类型。
...
- metadata.namespace：命名空间的字符串（默认为 default）。

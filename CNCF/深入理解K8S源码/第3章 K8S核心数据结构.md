...
- 作用：表示核心 API 组（Core Group / Legacy Group）支持的版本列表。这个组比较特殊，没有显式的组名（路径前缀是 `/api`而不是 `/apis/<group>`），它包含了像 pods，services，nodes 等最基础的核心资源。
- 核心字段：
...
客户端访问 `/apis`时必须能理解返回的是什么，才能继续发现每个组内的资源。其核心职责就是返回一个 `groups`数组。即使未来 Kubernetes 加入几百个新 API 组，这个结构也足够表示，无需改变格式。
...
这是 API 发现的最终目标：告诉客户端在某个特定 `group/version`下能操作哪些资源（`deployments`, `daemonsets`），以及每个资源的基本元信息（`kind`, `name`, `namespaced`, `verbs`等）。
...
- 外部版本： 对外暴露给用户请求的接口所使用。例如， 用户在通过 YAML 或 JSON 格式的描述文件创建资源时，外部版本资源通过资源版本（alpha、 beta、 stable）标识。
...
在 Kubernetes API 中，我们经常使用 GVK 或 GVR 来区分 Kubernetes 资源。其中 GVK 是 Group Version Kind 的简称，而 GVR 则是 Group Version Resource 的简称。
...
通过获取资源的 JSON 或 YAML 格式的序列化对象，进而从中获取该资源的 GVK。 RESTMapper 作为 GVK 到 GVR 的映射，通过 GVK 可以获取资源对象的 GVR，进而构建 RESTful API 请求以获取对应的资源。关于 RESTMapper 的知识，请参考 3.9 节。  
...
Kubernetes 内置了众多“资源组/资源版本/资源类型”，才有了现在功能强大的资源管理系统。用户可以通过以下方式获得当前 Kubernetes 支持的资源。
...
以 Deployment 资源为例，它的内部版本定义在 `k8s.io/kubernetes/pkg/apis/apps/` 目录下。内部版本的资源代码结构如下。

```shell
root@debian:~/golang/src/github.com/onexstack/kubernetes/pkg/apis/apps# tree -L 2
.
|-- OWNERS
|-- doc.go
|-- fuzzer
...
`k8s.io/kubernetes/pkg/apis/apps/{v版本}` 的核心职责如下：

- **代码生成粘合剂：** 各自的 `doc.go`文件通过指令 (`// +k8s:conversion-gen...`, `// +k8s:defaulter-gen...`) 引导代码生成器，明确告诉它们：

  - 需要与哪个内部版本包 (`pkg/apis/apps`) 进行转换。
  - 外部版本类型定义在哪里 (`k8s.io/api/apps/v1`)。
  - 如何生成默认值函数。
...
- **版本转换：** 是外部版本与内部版本进行相互转换的实现层。它包含：
  - `zz_generated.conversion.go`: 由 `conversion-gen` 自动生成的转换函数代码。
  - `conversion.go`(可选): 存放无法自动生成或需要特殊逻辑的手动编写的转换函数。
  - `conversion_test.go`: 对转换逻辑的单元测试。
- **默认值设置：**
  - `zz_generated.defaults.go`: 由 `defaulter-gen` 自动生成的默认值设置函数代码。这些函数在 API 对象被创建或更新时调用，为未指定的字段填充默认值。
  - `defaults.go`(可选): 存放无法自动生成或需要特殊逻辑的手动编写的默认值设置逻辑。
  - `defaults_test.go`: 对默认值设置逻辑的单元测试。
- **Scheme 集成：** `register.go`负责将生成的转换函数注册到 Kubernetes 的 `runtime.Scheme`中。这样，API 服务器就知道如何在内部版本和外部版本之间自动进行转换。



## 外部版本定义

Kubernetes 将公开 API 类型放在 `k8s.io/api` 模块中，使其与核心实现解耦，从而便于外部开发者独立使用，而无需依赖整个 `k8s.io/kubernetes` 项目。

以 Deployment 资源为例，它的外部版本定义在 `k8s.io/api/apps/` 目录下。外部版本的资源代码结构如下：

```shell
root@debian:~/golang/src/github.com/onexstack/kubernetes/staging/src/k8s.io/api/apps# tree -L 2
.
|-- OWNERS
|-- v1
|   |-- doc.go

...
说起 GitOps，可能马上会联想到我们前面讲的 DevOps，那么 GitOps 和 DevOps 之间有什么关系、又有什么区别呢？

DevOps 是一种文化，DevOps 包含了 Development 和 Operations 两个部分，是一种重视“软件开发人员（Dev）”和“IT运维技术人员（Ops）”之间沟通合作的文化或惯例。

如今很多团队当中都在践行着 DevOps 文化，其中核心的一条理念就是“自动化一切可自动化的”。而 GitOps，也正是基于这种理念下而诞生的一种持续交付方式。

# 二、为什么需要GitOps

GitOps 核心思想是将应用系统的声明性基础架构和应用程序存放在 Git 版本库中。

将 Git 作为交付流水线的核心，通过使用 Git，开发人员可以更高效地将注意力集中在业务功能而不是运维相关任务上。同时，GitOps 还是一整套 CI/CD 流水线方案。

在 GitOps 中可以自由地为流水线的不同部分选择最佳工具。

# 三、GitOps怎么实现

## 3.1实现方式介绍

通过 gitlab 实现 CI 流程，将 CD 部分使用 Argo CD 来完成，以实现应用部署及应用部署回滚的可控性。

<img src="images/git_argo.png" alt="git_argo" style="zoom: 50%;" />

## 3.2 环境准备

### 3.2.1主机规划

<img src="gitops.assets/image-20220908104352788.png" alt="image-20220908104352788" style="zoom:50%;" />

| 序号 | 主机名       | 主机ip        | 主机功能                | 软件                           |
| ---- | ------------ | ------------- | ----------------------- | ------------------------------ |
| 1    | dev          | 192.168.10.1  | 开发者 项目代码 apidemo | go、goland                     |
| 2    | gitlab       | 192.168.10.11 | 代码仓库，CI操作        | git-lab、git、go、docker       |
| 3    | harbor       | 192.168.10.21 | 存储和管理容器镜像      | docker、docker-compose、harbor |
...
<img src="gitops.assets/image-20220908104623080.png" alt="image-20220908104623080" style="zoom:50%;" />
...
32 external_url 'http://192.168.10.11'
~~~
...
浏览器输入 gitlab-ce 的服务器地址：http://192.168.10.11
...
当开发者 push 代码以后，gitlab-runner 会在 gitlab-ce 服务器执行流水线里定义的具体操作，流水线步骤里定义了gitlab-runner 通过 kustomize 客户端工具修改应用部署资源清单文件里的容器镜像版本，并重新 push 代码到代码仓库，所以需要在 gitlab-ce 服务器安装 git
...
本次应用 demo 是用 go 语言开发的，gitlab-runner 在编译项目源码时需要使用 go 命令，所以需要安装 go 环境
...
gitlab-runner 在执行流水线时需要将源码编译后的可执行文件制作成容器镜像，所以需要安装 docker
...
浏览器输入安装 harbor 配置文件里的 hostname，http://192.168.10.21，用户名为admin，密码为配置文件里设置的Harbor12345
...
MetalLB 可以为 kubernetes 集群中的 Service 提供负载均衡功能。

MetalLB 两大功能为:

- 地址分配，类似于DHCP
- 外部通告，一旦 MetalLB 为服务分配了外部IP地址，它就需要使群集之外的网络意识到该IP在群集中“存在”。MetalLB 有两种模式来实现此目的：ARP/NDP或BGP。
...
# 192.168.10.90-192.168.10.100 与集群节点IP同一网段      
...
# ingress-nginx-controller已获取到metallb分配的 EXTERNAL-IP
```
...
Argo([https://argoproj.github.io/projects/argo](https://www.oschina.net/action/GoToLink?url=https%3A%2F%2Fargoproj.github.io%2Fprojects%2Fargo)) 项目是一组 Kubernetes 原生工具集合，用于管理 Kubernetes 上的作业和应用程序。所有的 Argo 工具都实现为控制器和自定义资源。

**为什么选用Argo CD**

应用程序的定义、配置和环境应该是声明性的，并受版本控制。应用程序的部署和生命周期管理应该是自动化的、可审计的、易于理解的。
...
注意：如果 argocd-dex-server-7f75d56bc6-xqzqj，argocd-redis-5dff748d9c-rknln 两个pod无法拉取镜像的话需要修改Argocd部署方式
...
这里有个待解决的疑问：ingress中的ADDRESS这个字段值如何来的，因为今天自己学习 K8S基础/Ingress.md 时发现这个字段并没有值。
...
apiVersion: v1
data:
  password: clpYZ3d4dkY4TGlmMTV4Qw==  #此处为编码后密码，需要解码才能使用。
kind: Secret
metadata:
...
> 通过客户端可以进行登录Argocd命令行、直接修改admin登录密码等操作。
...
即使 Harbor 是镜像仓库，但 Harbor 服务器本身也需要：运行其他容器化服务、进行镜像构建和测试

```shell
# Harbor 服务器上可能执行的操作示例：

# 1. 构建镜像并推送到本地 Harbor
docker build -t 192.168.10.21/library/myapp:latest .
docker push 192.168.10.21/library/myapp:latest

# 2. 从 Harbor 拉取镜像运行测试
docker pull 192.168.10.21/library/nginx:latest
docker run -d 192.168.10.21/library/nginx:latest
```
...
# k8s其他节点重启containerd
[root@k8s-worker01 ~]# systemctl restart containerd
...
#runner实例描述，这里设置为apidemo，意为这个runner为apidemo项目的runner。可以填写任意字符串，方便识别就好。
[gitlab]: apidemo 
...
<img src="gitops.assets/image-20220907132058971.png" alt="image-20220907132058971" style="zoom:60%;" />

![runner配置01](images/runner配置01.png)

![runner配置02](images/runner配置02.png)

#### GitLab Runner 的三种类型（作用域）

Runner 根据其注册令牌的来源，分为三种类型，决定了它可以为哪些项目执行任务：

1. 特定 Runner（Specific Runner）
   - 描述：使用某个具体项目的设置 -> CI/CD -> Runners 页面中的令牌注册的 Runner。
   - 作用范围：默认只能为注册它的那个项目执行任务。
   - 您当前的情况：您使用的令牌是 `http://192.168.10.11/`上某个项目的令牌（如 `apidemo`项目），所以这个 Runner 目前是 `apidemo`项目的特定 Runner。
2. 群组 Runner（Group Runner）
   - 描述：使用某个群组的设置 -> CI/CD -> Runners 页面中的令牌注册的 Runner。
   - 作用范围：可以为该群组下的所有项目（包括子群组中的项目）执行任务。这对于有多个微服务或相关项目的团队非常有用。
3. 共享 Runner（Shared Runner）
   - 描述：使用 GitLab 实例管理员在管理员区域 -> 概览 -> Runners 页面找到的令牌注册的 Runner。
   - 作用范围：可以为整个 GitLab 实例中所有允许使用共享 Runner 的项目执行任务。这是最方便的配置，但通常需要管理员权限。
...
![image-20220907134513508](gitops.assets/image-20220907134513508.png)

<img src="images/配置go环境变量-01.png" alt="配置go环境变量-01" style="zoom:50%;" />
...
进入“资源”目录下，双击goland安装程序，安装goland

### 3.5.4 IDE中打开项目

#### 3.5.4.1 打开goland，选择项目

<img src="images/打开go项目-01.png" alt="打开go项目-01" style="zoom:50%;" />

<img src="images/打开go项目-02.png" alt="打开go项目-02" style="zoom:50%;" />
...
metadata:
  ...
  namespace: apidemo
  annotations:
    ingressclass.kubernetes.io/is-default-class: "true" # 它不应该出现在 Ingress 资源中，而应该出现在IngressClass资源中。
    kubernetes.io/ingress.class: nginx
spec:
  rules:
...
apiVersion: kustomize.config.k8s.io/v1beta1 # 指定 Kustomize 的 API 版本
kind: Kustomization  # 资源类型，固定为 Kustomization
```
...
workflow:  # 设置工作流规则
  rules:
    - if: $CI_COMMIT_BRANCH == 'master'  # 只有 master 分支的提交才会触发流水线
      # 为 master 分支的流水线设置镜像仓库认证信息
      variables:
...
variables:  # 全局变量定义
  ...
  app_version: $CI_COMMIT_SHORT_SHA # app版本号，使用 Git 提交的短哈希作为版本号（确保每次构建版本唯一），gitlab内置变量
  namespace: $CI_PROJECT_NAMESPACE # 项目所在组，使用 GitLab 项目组作为 K8s 命名空间，gitlab内置变量
  GIT_CLONE_PATH: '$CI_BUILDS_DIR/$CI_JOB_ID/$CI_PROJECT_NAME' # 自定义代码克隆路径，避免路径冲突
...
build code: # 代码编译作业
  stage: build # 此作业为build阶段的作业
  script:
    - go build -o apidemo # 具体执行的命令，此处为编译go项目，编译完成会产生apidemo可执行文件
  artifacts: # gitlab流水线文件内置关键字，作用为保留制品
    paths: # 保留制品所在位置
      - apidemo # 当前步骤需要保留的制品文件，提供给下一步骤使用

docker build:
  stage: build
  script:
    - docker build -t $app_name:$app_version .  # 此步骤通过Dockerfile文件构建镜像
  needs: # gitlab流水线文件内置关键字，此job所依赖的job，只有当被依赖的job完成后，此job才会执行
    - build code
...
deploy dev:
  stage: deploy
  before_script: # runner通过kustomize来修改镜像版本信息，所以需要在修改镜像版本信息后，重新push代码
    # Git 认证配置：使用用户名密码设置远程仓库地址（包含认证信息）
    # 提交者信息：配置 Git 提交时显示的用户信息
  ...
  script:
    # 切换到 master 分支，如果分支不存在则创建
    - git checkout -B master
    # 进入资源清单文件目录
    - cd base
    # runner通过kustomize工具修改镜像版本信息
    - kustomize edit set image $registry/$app_name/$app_name:$app_version
    # 打印修改后的配置文件，便于调试和日志查看
    - cat kustomization.yaml
    # [skip ci]关键作用：告诉 GitLab 跳过此次提交的 CI 触发，防止死循环
...
`GIT_CLONE_PATH`是一个 GitLab CI/CD 的预定义变量，用于自定义 Git 仓库的克隆路径。

```shell
GIT_CLONE_PATH: '$CI_BUILDS_DIR/$CI_JOB_ID/$CI_PROJECT_NAME'

分解各个部分：
$CI_BUILDS_DIR: GitLab Runner 的工作目录（默认：/home/gitlab-runner/builds）
$CI_JOB_ID: 当前作业的唯一ID（每次运行都不同）
$CI_PROJECT_NAME: 项目名称
```

每个作业有独立的克隆目录（因为 `$CI_JOB_ID`是唯一的），避免并行作业间的文件冲突。

```shell
/home/gitlab-runner/builds/
├── 12345678/                    # Job 1 的独立目录
│   └── apidemo/
│       ├── src/
│       ├── base/kustomization.yaml
│       └── .git/
├── 12345679/                    # Job 2 的独立目录  
│   └── apidemo/
│       ├── src/
│       ├── base/kustomization.yaml  
│       └── .git/
└── 12345680/                    # Job 3 的独立目录
    └── apidemo/
        ├── src/
        ├── base/kustomization.yaml
        └── .git/
...
test-unit:
  variables:
    GIT_CLONE_PATH: '$CI_BUILDS_DIR/$CI_JOB_ID/$CI_PROJECT_NAME'
  script:
    - modify config.json  # 不会影响其他作业

test-integration:
  variables:
    GIT_CLONE_PATH: '$CI_BUILDS_DIR/$CI_JOB_ID/$CI_PROJECT_NAME'  
  script:
    - modify config.json  # 独立的修改
```
...
```yaml
variables:
  GIT_CLONE_PATH: '$CI_BUILDS_DIR/$CI_PROJECT_NAME'  # 项目级共享

build:
  script: 
    - make build  # 所有作业共享同一克隆
...
1. 在作业工作目录中查找 `apidemo`文件（作业工作目录为：GIT_CLONE_PATH）
2. 将文件打包压缩
3. 上传到 GitLab 服务器（或配置的存储后端）
4. 在流水线界面提供下载链接

后续作业可以通过以下方式获取制品：

- 隐式传递：默认情况下，同一流水线的后续作业会自动下载之前所有作业的制品
- 显式依赖：使用 `needs`或 `dependencies`精确控制
...
docker build:
  ...
  needs:
    - job: build code
      artifacts: true  # ✅ 关键配置：明确要求制品
  script:
    - ls -la apidemo  # 确认文件存在
    - docker build -t $app_name:$app_version .
    
# 当制品正确传递时，文件会出现在当前作业的工作目录中
```
...
<img src="images/gitlab配置环境变量-03.png" alt="gitlab配置环境变量-03" style="zoom:50%;" />

<img src="images/gitlab配置环境变量-04.png" alt="gitlab配置环境变量-04" style="zoom:50%;" />

<img src="images/gitlab配置环境变量-05.png" alt="gitlab配置环境变量-05" style="zoom:50%;" />

<img src="images/gitlab配置环境变量-06.png" alt="gitlab配置环境变量-06" style="zoom:50%;" />

<img src="images/gitlab配置环境变量-07.png" alt="gitlab配置环境变量-07" style="zoom:50%;" />

### 3.5.9 开发者环境添加远程地址仓库

<img src="images/本地添加远程仓库.png" alt="本地添加远程仓库" style="zoom:67%;" />
...
![image-20220907224523395](gitops.assets/image-20220907224523395.png)

![image-20220907224746513](gitops.assets/image-20220907224746513.png)

![image-20220907224925515](gitops.assets/image-20220907224925515.png)

![image-20220907224948483](gitops.assets/image-20220907224948483.png)

![image-20220907225007964](gitops.assets/image-20220907225007964.png)
...
![image-20220907230140527](gitops.assets/image-20220907230140527.png)

![image-20220907230348772](gitops.assets/image-20220907230348772.png)

![image-20220907230413912](gitops.assets/image-20220907230413912.png)

![image-20220907230610324](gitops.assets/image-20220907230610324.png)

![image-20220907230632019](gitops.assets/image-20220907230632019.png)

![image-20220907230654857](gitops.assets/image-20220907230654857.png)

![image-20220907230703654](gitops.assets/image-20220907230703654.png)

![image-20220907230715094](gitops.assets/image-20220907230715094.png)



![image-20220907230729871](gitops.assets/image-20220907230729871.png)
...
- `containerd`（CNCF 项目，Kubernetes 默认运行时）：
  - 源于 Docker 的组件拆分，负责镜像管理、存储和容器生命周期。
...
而 Docker 将容器操作都迁移到 containerd 中去是因为做 Swarm，想要进军 PaaS 市场，做了这个架构切分，让 Docker Daemon 专门去负责上层的封装编排，当然后面的结果我们知道 Swarm 在 Kubernetes 面前是惨败，然后 Docker 公司就把 containerd 项目捐献给了 CNCF 基金会，这个也是现在的 Docker 架构。

...
当然最火大家最熟悉的容器运行时非 Docker 莫属了，Docker 凭借一己之力大大推进了容器技术的普及。当然随着容器生态的不断发展，业界也慢慢出现了其他的运行时，比如 Containerd、kata container、cri-o 等等，这些运行时提供的功能不尽相同，有些只有容器运行的功能，有些除运行容器之外还提供了容器镜像的管理功能。根据容器运行时提供的功能，我们可以将容器运行时分为低层运行时和高层运行时。

**低层运行时（Low-level Runtime）**

功能定位：直接与操作系统交互，实现容器的基础隔离与资源限制，核心依赖 Linux 内核的 **Namespace**（进程、网络、文件系统隔离）和 **Cgroups**（CPU、内存等资源限制）。
...
- `runv`（基于虚拟化）：
  - 通过 Hypervisor（如 KVM）为每个容器分配独立内核，实现强隔离（如 Kata Containers）。
  - 适用于安全敏感场景（如公有云多租户）。
- `gVisor`（用户态内核）：
  - 拦截系统调用，在用户态模拟内核行为，兼顾安全性与性能。

**高层运行时（High-level Runtime）**

功能定位：管理容器镜像、实现 CRI（Container Runtime Interface）规范、提供网络/存储插件集成等高级功能。

代表项目：

- `containerd`（CNCF 项目，Kubernetes 默认运行时）：
  - 源于 Docker 的组件拆分，负责镜像管理与存储、容器生命周期。
  - 内置 CRI 插件，直接对接 Kubelet，无需额外 Shim 层（相比 Docker 调用链更短）。
- `CRI-O`（专为 Kubernetes 设计）：
  - 仅实现 CRI 接口，轻量化设计，避免冗余功能。
  - 直接调用 `runc` 运行容器，与 Kubernetes 集成更紧密。
- `Docker Engine`（完整套件）：
...
- ImageService 主要定义拉取、查看和删除镜像等操作。
...
root@eden:~# nerdctl images
REPOSITORY                                     TAG        IMAGE ID        CREATED        PLATFORM       SIZE       BLOB SIZE
k8s.gcr.io/kube-apiserver                      v1.17.3    33400ea29255    3 hours ago    linux/amd64    176.5MB    50.63MB
nginx                                          alpine     b2e814d28359    3 hours ago    linux/amd64    56.83MB    22.41MB
registry.k8s.io/sig-storage/csi-provisioner    v3.5.0     d078dc174323    3 hours ago    linux/amd64    69.78MB    28.35MB
```

...
- `serviceAccountToken` 数据源：这个数据源是一个时间限制的服务账户令牌，由 `kubelet` 使用 TokenRequest API 从 `kube-apiserver` 获取。这个令牌是临时的，并且会在 Pod 被删除或者令牌本身的生命周期结束后过期。这个过期机制增加了安全性，因为即使令牌被泄漏，它也只在有限的时间里有效。这种机制取代了以前的做法，即通过 Secret 提供不会过期的服务账户令牌。

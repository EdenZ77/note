...
root@master:~/yamlDir/rbac# kubectl api-resources
...
## GVK
要通过 YAML 表达一个资源对象需要指定 `group`、`kind` 和 `version`，这个在 Kubernetes 的 API Server 中简称为 GVK。
...
- RoleBinding 和 ClusterRoleBinding：角色绑定和集群角色绑定，简单来说就是把声明的 Subject 和 Role 进行绑定的过程（给某个对象绑定上操作的权限），二者的区别也是作用范围的区别：RoleBinding 只会影响到当前 `namespace` 下面的资源操作权限，而 ClusterRoleBinding 会影响到所有的 `namespace`。
...
- context:
    cluster: kubernetes
    namespace: kube-system
    # 当您使用 cnych-context 上下文时，所有的 kubectl 命令（如 kubectl get pods）会自动添加 -n kube-system 参数，相当于默认在 kube-system 命名空间中执行操作。
    user: cnych
  name: cnych-context
- context:
    cluster: kubernetes
...
metadata:
  name: cnych-role
  # kube-system 指明了这个Role资源对象将被创建在哪个命名空间中，并且更重要的是，它定义了此Role所拥有的权限规则（rules）的生效范围仅限于kube-system这个命名空间
  namespace: kube-system
rules:
  - apiGroups: ["", "apps"]
...
metadata:
  ...
  # kube-system指明了这个RoleBinding资源本身将被创建在kube-system命名空间中，并且，它定义了此次授权绑定的生效范围仅限于kube-system命名空间。
  # RoleBinding 和它引用的 Role 必须在同一个命名空间
  namespace: kube-system
subjects:
  - kind: User
    name: cnych
...
`subjects.apiGroup` 字段用于指定引用的主体（Subject）所属的 API 组。
...
当创建 Pod 的时候规范下面有一个 `spec.serviceAccount` 的属性用来指定该 Pod 使用哪个 ServiceAccount，如果没有指定的话则默认使用 `default` 这个 `sa`（每个 `namespace`下面都有一个 `default` 这个 `sa`），然后通过投射卷，在容器的目录 `/var/run/secrets/kubernetes.io/serviceaccount/` 下有一个 `token` 令牌文件。我们通过 RBAC 对该 `sa` 授予了什么权限，那么容器里的应用拿着这个 `token` 后就具备对应的权限。
...
eyJhbGciOiJSUzI1NiIsImtpZCI6Il81VGI3bDBRX0lwMVUyNFh0YUY1Sld1c1JwbUFEYzRmRFFCamlGbWxLS0UifQ..Xco0O5xTHqvLVgwUzfbVCxbDNqKsKcxPO1UccP5zvPKjg18x5Dnnz2yR3pLKUMVbxdiJRJf6w1FLEoGtrvurmc6VRDgNEL0NIxBzHH_sz-XXEMxJAOq_5IUpzO1T2wJqb4nzLBk6TEU2EVUb0g0tRy_rCVs2eMw1xSyFr_J3_iO5XAHGk-Kb5w1iGWNzYAlzm4KoJsZ-3bslHzZ5UsS3uwXnlJnsuVFRnuugzmAOXIDql5WgqEYK4LRQg_lWh6DnVBDToTtZZygxxspUruiSlF86hV0oFyoRd1EE0XpY79tTUW-PnUHppHW2d8mWXMOOv-42aKlX1VUHoaybIh_zHw
```
可以看到 Pod 里通过投射卷所挂载的 `token` 跟 `sa-demo` 对应的 `secret` 包含的 `token` 是一样的。

如果需要在 Pod 中去访问 K8s 集群的资源对象，现在就可以为使用的 `sa` 绑定上相应的权限，然后在 Pod 应用中使用对应的 `token` 去和 APIServer 进行通信就可以了，这个时候的 `token` 就具有对应的权限。
...
- `serviceAccountToken` 数据源：这个数据源是一个有时间限制的服务账户令牌，由 `kubelet` 使用 TokenRequest API 从 `kube-apiserver` 获取。这个令牌是临时的，并且会在 Pod 被删除或者定义的生命周期（默认1个小时）结束后过期。这种机制取代了以前的做法，即通过 Secret 提供不会过期的服务账户令牌。

- `configMap` 数据源：ConfigMap 通常用来存储配置数据，目前存储 ca.crt，用来验证与 `kube-apiserver` 通信的证书，确保 Pod 在与 API 服务器通信时不会被中间人攻击或被引导到错误的服务器。它提供了集群的根证书颁发机构的证书，Pod 可以信任这个证书颁发机构签发的所有证书。
...
......
  ...
  - name: kube-api-access-k298c
    projected:
      ...
      sources:
      - serviceAccountToken:
          expirationSeconds: 3607 # 60 分 7 秒
          path: token
      - configMap:
          items:
          - key: ca.crt
...
```
然后怎么去验证这个 ServiceAccount 呢？（在下面的“完整测试”章节可以通过 Pod 中的容器调用 API 接口来测试）以前的版本 ServiceAccount 会自动生成一个 Secret 对象和它进行映射，这个 Secret 里面包含一个 `token`，可以利用这个 `token` 去登录 Dashboard，然后就可以在 Dashboard 中来验证我们的功能是否符合预期了，但是现在的版本默认已经不支持这种方式了。

我们可以手动创建一个长久有效的 ServiceAccount 令牌，要为 ServiceAccount 创建一个不过期、持久化的API 令牌，需要创建一个类型为 `kubernetes.io/service-account-token` 的 Secret，附带引用 ServiceAccount 的注解。控制平面随后生成一个长久的令牌，并使用生成的令牌数据更新该 Secret，比如这里可以创建一个如下所示的 Secret 资源对象：
```yaml
# cnych-token.yaml
apiVersion: v1
kind: Secret
...
可以看到可以访问 `pod` 列表了，但是也会有一些其他额外的提示：`statefulsets.apps is forbidden: User "system:serviceaccount:kube-system:cnych-sa" cannot list resource "statefulsets" in API group "apps" in the namespace "kube-system"` ，这是因为当前登录用户只被授予访问 `pods` 和 `deployments` 的权限。

## 可以全局访问的 ServiceAccount
如果现在创建一个新的 ServiceAccount，需要操作的权限作用于所有的 `namespace`，这个时候就需要使用到 ClusterRole 和 ClusterRoleBinding 这两种资源对象了。同样，首先新建一个 ServiceAcount 对象：
```yaml
# cnych-sa2.yaml
apiVersion: v1
kind: ServiceAccount
...
apiVersion: rbac.authorization.k8s.io/v1
metadata:
  name: cnych-sa2-clusterrolebinding # cluster级别的role和rolebindings没有namespace属性
subjects:
  - kind: ServiceAccount
    name: cnych-sa2
    namespace: kube-system
...
1. 创建 ServiceAccount
2. 为这个 ServiceAccount 创建相应的 RBAC 权限（Role 或 ClusterRole 以及 RoleBinding 或 ClusterRoleBinding）—— 这一步不是必须的，但通常我们会赋予权限
3. 在 Pod 的 spec 中指定 serviceAccountName 为 sa-demo
...
# 当前并没有给sa-demo设置权限，只有默认的权限；进入nginx容器测试如下：
# 没有权限获得default命名空间下面的Pod列表
root@demo:/var/run/secrets/kubernetes.io/serviceaccount# curl -H "Authorization: Bearer $(cat /var/run/secrets/kubernetes.io/serviceaccount/token)" \
     ...
     https://kubernetes.default.svc/api/v1/namespaces/default/pods
{
  ...
  "metadata": {},
  "status": "Failure",
  "message": "pods is forbidden: User \"system:serviceaccount:default:sa-demo\" cannot list resource \"pods\" in API group \"\" in the namespace \"default\"",
  "reason": "Forbidden",
  "details": {
    "kind": "pods"
  },
...
# 如果请求不加 Authorization 和 --cacert 则使用的匿名User
root@demo:/var/run/secrets/kubernetes.io/serviceaccount# curl -k https://kubernetes.default.svc/api/v1/namespaces/default/pods
{
  "kind": "Status",
  "apiVersion": "v1",
  "metadata": {},
...
# 下面的访问具有权限，但是如果使用匿名User：curl -k https://kubernetes.default.svc/api 则没有权限
root@demo:/var/run/secrets/kubernetes.io/serviceaccount# curl -H "Authorization: Bearer $(cat /var/run/secrets/kubernetes.io/serviceaccount/token)"      --cacert /var/run/secrets/kubernetes.io/serviceaccount/ca.crt      https://kubernetes.default.svc/api
{
  "kind": "APIVersions",
  "versions": [
    "v1"
  ...
  "serverAddressByClientCIDRs": [
    {
      ...
      "serverAddress": "192.168.220.152:6443"
    }
  ]
}
...
# 再次进入nginx容器测试：可以看到具有了权限
root@demo:/var/run/secrets/kubernetes.io/serviceaccount# curl -H "Authorization: Bearer $(cat /var/run/secrets/kubernetes.io/serviceaccount/token)"      --cacert /var/run/secrets/kubernetes.io/serviceaccount/ca.crt      https://kubernetes.default.svc/api/v1/namespaces/default/pods
{
  "kind": "PodList",
  "apiVersion": "v1",
  "metadata": {
    "resourceVersion": "1595425"
  },
  "items": [
    {
      "metadata": {

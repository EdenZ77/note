  ...
  volumes:
    - name: config-volume
      configMap:
        ...
        items:
          - key: mysql.conf
            path: path/to/msyql.conf # 相对于卷根目录的文件路径，用来映射 ConfigMap 中的键
  containers:
    - name: testcm4
      image: busybox
...
在这个 ConfigMap 中，`conf1` 和 `conf2` 是子目录，每个子目录中包含多个文件。假设你想将 `conf1` 子目录中的 `file1.conf` 文件挂载到容器内的 `/etc/app/file1.conf` 路径：

...
它作为一个集群内的网络组件，为裸金属 K8s 集群提供了与云平台类似的负载均衡器实现。它主要做了两件事：

1. IP 地址分配（Address Allocation）： 为 `LoadBalancer`类型的 Service 分配一个 IP 地址。
2. 外部公告（External Announcement）： 让集群外的网络知道这个 IP 地址在集群内，并将流量引导至集群节点。

MetalLB 通过两种模式来实现第二项功能（外部公告），这也是其工作原理的核心。

### MetalLB 的两种运作模式

#### 模式一：Layer 2 模式（默认且最常用）

这是最简单、最容易理解的模式，它基于标准的地址解析协议（ARP）和邻居发现协议（NDP）。

> ARP 和 NDP 都是用于将网络层地址（如 IP 地址）解析为数据链路层地址（如 MAC 地址）的协议，但它们分别用于不同的 IP 版本：ARP 是 IPv4 网络中的协议，而 NDP 是 IPv6 网络中的协议。

工作原理（以 IPv4/ARP 为例）：

1. 用户创建 Service： 你创建一个 `type: LoadBalancer`的 Service。
2. MetalLB 分配 IP： MetalLB 的 `controller`组件从其配置的 IP 地址池（Pool）中选取一个空闲的 IP，并将其分配给该 Service。这个 IP 就是 Service 的 `EXTERNAL-IP`。
3. 选举领导者节点： 在同一子网内，MetalLB 的 `speaker`组件（运行在每个节点上）会通过 Leader Election 机制选出一个节点作为“领导者”，负责响应这个 IP 的 ARP 请求。
4. 响应 ARP 请求：当集群外部的客户端要访问 `EXTERNAL-IP`时，它会在本地网络中广播一个 ARP 请求，询问：“谁的 IP 是 `EXTERNAL-IP`？” 只有被选举出的那个领导者节点会用自己的 MAC 地址来响应这个 ARP 请求。也就是说，它“对外宣称”：“这个 IP 是我的！”
5. 流量路由：网络交换机/路由器学习到 `EXTERNAL-IP`对应的是领导者节点的 MAC 地址。所有发往 `EXTERNAL-IP`的网络流量都会被二层网络设备（交换机）导向这个特定的领导者节点。
6. 节点内负载均衡：领导者节点的 `kube-proxy`已经预先设置了 iptables/IPVS 规则。当流量到达领导者节点后，这些规则会将流量负载均衡到该 Service 对应的所有健康 Pod 上（这些 Pod 可能分布在集群的任意节点上）。

MetalLB Layer 2 模式分配的 `EXTERNAL-IP`，必须与 Kubernetes 集群的节点（Node）处于同一个物理网络广播域中，也就是我们常说的同一个局域网段/VLAN。

**所以，“在同一子网内”意味着：Service 的 External-IP、所有 Kubernetes 节点、以及要访问这个 Service 的外部客户端，都必须位于同一个二层网络环境中。这是 Layer 2 模式正常工作的前提条件。**

Layer 2 模式的特点：

- 优点：
  - 简单通用： 不需要特殊的网络设备支持，能在任何支持 ARP/NDP 的以太网环境中工作。
  - 容错： 如果领导者节点宕机，MetalLB 会快速（通常在几秒内）进行故障转移，选举新的领导者节点来接管 IP 并响应 ARP 请求。
- 缺点：
  - 单节点瓶颈： 所有入口流量都必须先经过领导者节点。虽然最终的 Pod 负载均衡是跨所有节点的，但入口流量本身会集中在一个节点上。这个节点的网络带宽可能成为瓶颈。
  - 故障转移速度： 故障转移速度依赖于 ARP 缓存过期时间（通常需要几分钟），虽然 MetalLB 有办法加速这个过程，但不如 BGP 模式快。

#### 模式二：BGP 模式（更高级，性能更好）

BGP（边界网关协议）是互联网核心的路由协议。大型数据中心通常使用 BGP 在路由器之间交换路由信息。

工作原理：

1. 用户创建 Service： 同样，你创建一个 `LoadBalancer`Service。
2. MetalLB 分配 IP： 同样，Controller 从 IP 池中分配一个 IP 给 Service。
3. 发布 BGP 路由通告：集群每个节点上运行的 `speaker`组件都会与一台或多台物理网络设备（BGP 对等体/路由器）建立 BGP 会话。每个 `speaker`都会向路由器通告一条路由规则：“要到达 `EXTERNAL-IP`，下一跳地址是我这个节点的 IP。”
4. 流量路由：物理路由器通过 BGP 学习到了多条通往 `EXTERNAL-IP`的路由（每个节点通告一条）。路由器会使用等价多路径路由（ECMP） 技术，将去往 `EXTERNAL-IP`的流量大致均匀地分发到所有健康的、通告了该路由的 K8s 节点上。
5. 节点内负载均衡： 和 Layer 2 模式一样，流量到达任一节点后，由该节点的 `kube-proxy`规则负责转发到具体的 Pod。

BGP 模式的特点：

- 优点：
  - 真正的负载均衡： 入口流量被路由器通过 ECMP 分发到所有集群节点，避免了 Layer 2 模式的单节点瓶颈，带宽可以线性扩展。
  - 快速的故障转移： 如果某个节点宕机，BGP 会话会中断，路由器会在几秒钟内从路由表中移除该节点通告的路由，并将流量无缝切换到其他健康节点。
- 缺点：
  - 网络设备要求： 需要网络基础设施（TOR 交换机或核心路由器）支持 BGP 并能与 MetalLB 建立对等会话。
  - 配置稍复杂： 需要配置 MetalLB 和网络设备双方的 BGP 参数（AS号、对等体IP等）。
...
| :------------------- | :---------------------------------------- | :------------------------------------- |
| **Speaker 工作状态** | 一个领导者工作，其他待命                  | 所有 Speaker 同时活跃工作              |
| **选举机制**         | 需要 Leader Election 选出一个节点响应 ARP | 不需要选举，所有节点都通告 BGP 路由    |
| **流量入口**         | 集中式：所有流量先到一个领导者节点        | 分布式：流量被路由器直接分发到所有节点 |
| **带宽瓶颈**         | 存在单节点瓶颈风险                        | 无单节点瓶颈，可利用所有节点的入口带宽 |
| **故障转移**         | 相对较慢（依赖 ARP 缓存）                 | 极快（BGP 路由收敛，通常几秒钟内）     |
| **网络要求**         | 简单，只需交换机（二层可达）              | 复杂，需要支持 BGP 和 ECMP 的路由器    |
...
1. Controller（控制器）：以 Deployment 形式部署，通常只有一个副本（Leader Election 确保高可用）。核心职责是监视 Kubernetes API，监听 Service 的变化。当发现有新的 `LoadBalancer`Service 时，负责从配置的 IP 地址池中分配和释放 IP 地址。
2. Speaker（宣告者）：以 DaemonSet 形式部署，在每个节点上运行一个副本。核心职责是使用特定协议（Layer 2 或 BGP）对外“公告”或“宣告” Service 的 IP 地址。在 Layer 2 模式下，负责 ARP/NDP 响应。在 BGP 模式下，负责与路由器建立会话并通告路由。

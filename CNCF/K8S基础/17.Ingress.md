...
四层负载均衡（如 Kubernetes 的 `Service`和 `LoadBalancer`）工作在 TCP/UDP 层，其核心任务是将网络流量引导至后端 Pod。这带来了以下几个方面的固有缺陷：

## 1. 应用层（七层）协议感知能力缺失

- 问题描述：L4 负载均衡器仅能识别 IP 地址和端口号，无法解析数据包中的应用层协议（如 HTTP/HTTPS）。
- 具体影响：
  - 无法进行高级路由：不能实现基于域名（FQDN）、URL 路径、HTTP 请求头（如 `User-Agent`）或 HTTP 方法（如 GET、POST）的精细路由。所有到达同一 IP 和端口的流量都会被无差别地转发。
  - 无法管理 SSL/TLS：不能提供统一的 SSL 证书管理、终止或卸载。每个应用需要自行处理繁琐的证书管理。
  - 不支持高级流量治理：无法实现基于 HTTP 错误码的重试、超时控制、熔断、基于权重的流量拆分（灰度发布）等现代微服务治理策略。

## 2. 服务暴露与网络管理效率低下

- 问题描述：每个需要对外暴露的服务通常都需要一个独立的 `LoadBalancer`Service，从而分配一个独立的外部 IP 地址。
- 具体影响：
  - IP 地址消耗巨大：在微服务架构下，服务数量众多，会导致对公网 IP 或内网 IP 的快速消耗，成本高昂且管理复杂。
  - 端口管理混乱：虽然可以使用同一 IP 的不同端口，但这不符合标准（如 HTTP 默认用 80/443 端口），对用户不友好且配置复杂。
  - 安全隐患：暴露过多的 IP 和端口，增加了网络攻击面，安全策略难以维护。

## 3. 健康检查机制较为粗放

- 问题描述：L4 的健康检查通常仅限于 TCP 连接检查。
- 具体影响：
  - 准确性不足：一个 Pod 可能能建立 TCP 连接，但其中的应用可能已死锁或无法正常处理业务请求（例如，返回 5xx 错误）。L4 检查无法识别这种“假活”状态，导致流量仍被转发到不健康的 Pod。
  - 无法实现精细引流：无法根据应用返回的特定 HTTP 状态码或响应体内容来判断健康状态。
...
Kubernetes 使用 Ingress 资源用于定义策略和一个提供路由转发服务的 Ingress Controller，二者相结合实现了灵活的7层路由功能。
...
<img src="image/image-20251120142833891.png" alt="image-20251120142833891" style="zoom:50%;" />
...
<img src="image/2024-03-07-11-12-00.png" style="zoom:30%;" />
...
例如，假设你有一个自定义的 Ingress 控制器参数资源，它可以定义一些日志设置或者安全策略。自定义资源对象（例如 `IngressParameters`）的结构、支持的字段和它们的含义都是由 Ingress 控制器的开发者定义的。所以你需要查看控制器文档来确定哪些参数可配置，并了解如何设置这些参数，指导你如何在自定义资源中设定这些选项。
...
然而，随着 Kubernetes 的发展，官方推荐使用 ingressClassName 字段来替代 kubernetes.io/ingress.class 注解。新创建的 Ingress 资源应该更倾向于使用这个字段。尽管如此，出于兼容的考虑，即使 kubernetes.io/ingress.class 注解已被官方废弃，Ingress 控制器仍然应该兼容这个注解。
...
由于一个集群中可能有多个 Ingress 控制器，所以我们还可以将一个特定的 IngressClass 对象标记为集群默认的 Ingress 类。只需要将一个 IngressClass 资源的 `ingressclass.kubernetes.io/is-default-class` 注解设置为 true 即可，这样未指定 ingressClassName 字段的 Ingress 就会使用这个默认的 IngressClass。
...
```
其中 ingress-nginx-controller-admission 是为准入控制器提供服务的，我们也是强烈推荐开启该准入控制器，这样当我们创建不合要求的 Ingress 对象后就会直接被拒绝了，另外一个 ingress-nginx-controller 就是 ingress 控制器对外暴露的服务，我们可以看到默认是一个 LoadBalancer 类型的 Service，我们知道该类型是用于云服务商的，我们这里在本地环境，暂时不能使用，但是可以通过它的 NodePort 来访问，后面我们会提供在本地测试环境提供 LoadBalancer 的方式。
...
```
这里只提供了一个 controller 属性，对应的值和 ingress-nginx 的启动参数中的 controller-class 一致的。
```sh
apiVersion: apps/v1
kind: Deployment
metadata:
...
```
注意我们这里配置的域名是 `first-ingress.192.168.58.176.nip.io`，该地址其实会直接映射到 `192.168.58.176` 上面，该 IP 地址就是我的 Master 节点地址(任意节点均可以)，因为我们这里 ingress 控制器是通过 NodePort 对外进行暴露的，所以可以通过 `域名:nodePort` 来访问服务。
...
`nip.io` 是由 PowerDNS 提供支持的开源服务，允许我们可以直接通过使用以下格式将任何主机名映射到主机IP地址，这样就不需要在 /etc/hosts 文件中配置映射了。

你的电脑向公共 DNS 查询 `first-ingress.192.168.58.176.nip.io`的 IP 地址。`nip.io`的 DNS 服务器识别出域名中的 `192.168.58.176`，直接返回这个 IP 地址作为解析结果。

...
可以指定一个“稳定窗口”，以防止扩缩目标的副本计数发生[波动](https://kubernetes.io/zh-cn/docs/tasks/run-application/horizontal-pod-autoscale/#flapping)。 扩缩策略还允许你在扩缩时控制副本的变化率。
...
- 字段:
  - stabilizationWindowSeconds (`<integer>`): 稳定窗口的时间（以秒为单位）。必须大于等于零且小于等于 3600 秒（1 小时）。如果未设置，默认值为 300 秒。
    - 默认值:
      - 对于向上扩展: 0 秒（即不使用稳定窗口）。
      - 对于向下扩展: 300 秒（即稳定窗口为 300 秒）
  ...
  - selectPolicy (`<string>`): 用于指定应使用的策略。如果未设置，默认值为 `Max`。
    - 取值:
      - Max: 选择最大值
      - Min: 选择最小值
      - Disabled: 禁用策略

...
详细原理见：CNCF\K8S专题进阶\02_kube-proxy\
...
## ExternalName
ExternalName 是 Service 的特例，它没有 `selector`，也没有定义 `ports`。
```yaml
kind: Service
apiVersion: v1
metadata:
...
```shell
# 在每个节点上，Kubernetes 会创建以下网络配置：

# 1. 在 kube-ipvs0 网卡上绑定 externalIP
$ ip addr show kube-ipvs0
4: kube-ipvs0: <BROADCAST,NOARP> mtu 1500 qdisc noop state DOWN group default 
    inet 203.0.113.100/32 scope global kube-ipvs0
...
外部客户端
     ↓
目标: externalIP:Port (203.0.113.100:80)
     ↓
网络路由 → 任意 Kubernetes 节点
     ↓
节点网络栈识别目标IP属于 kube-ipvs0
     ↓
IPVS 负载均衡 → 后端 Pod (10.244.1.5:8080)
     ↓
DNAT → 实际 Pod IP
```

必须确保 externalIP 可路由到至少一个节点。

当删除这个 Service 后，自动会删除 IPVS 转发规则，同时删除 kube-ipvs0 网卡上绑定的 externalIP。

## LoadBalancer

架构组成

<img src="image/1a26c850416ae.png" alt="1a26c850416ae" style="zoom:30%;" />

数据包流向分析

<img src="image/e35eed933264.png" alt="e35eed933264" style="zoom:30%;" />

LoadBalancer Service会自动创建NodePort！

- **ClusterIP**：基础，提供集群内部服务发现
- **NodePort** = ClusterIP + 节点端口映射
- **LoadBalancer** = NodePort + 云负载均衡器
...
- Cluster 表示：流量可以转发到其他节点上的 Pod。客户端的源 IP 地址会被伪装（SNAT）。
- Local 表示：如果设置为 Local ，将以一种假设外部负载平衡器将负责平衡 Node 之间的服务流量的方式配置服务，因此流量只会路由到接收流量的节点上的本地后端 Pod，而不伪装客户端源 IP。（错误地发送到没有端点的 Node 的流量将被丢弃。）

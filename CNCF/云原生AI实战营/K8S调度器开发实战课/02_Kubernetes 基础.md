...
![img](image/FqNKssx0OZad8X1-QViehY7IlJkh)
...
上述命令中，-v 10会指定 kubectl 非常详细的操作日志。为了让你更清晰的看到 kubectl create -f replicaset.yaml -v 10后，kubectl 的核心操作，我简化了 kubectl 的日志输出内容，并在日志中添加了注释，具体如下：
...
通过上面的日志，我们可以看到在 kubectl create -f replicaset.yaml -v 10具体执行了以下操作：
...
在调用 kubectl create -f replicaset.yaml请求 kube-apiserver 创建 ReplicaSet 资源后，kube-apiserver 会对请求进行认证、鉴权，对资源设置默认值、准入控制、参数校验等，并将资源数据保存在 Etcd 中。

我们这时候，可以通过 etcdctl工具查看 Etcd 中保存的资源数据：
...
etcdctl 命令中，-w指定了输出的格式，key 的值是经过 base64 编码，需要解码后才能看到实际值，如：
...
我们可以看到所有的 Kuberentes 的所有元数据都保存在/registry目录下，下一层就是API对象类型（复数形式），再下一层是namespace，最后一层是对象的名字。Kubernetes 使用了 Etcd V3 API。V3 版本的数据存储没有目录层级关系，而是采用了平展（flat）模式，换句话说 /a和 /a/b 并没有嵌套关系，而只是 key 的名字有差别而已，这个和 AWS S3 及 OpenStack Swift 对象存储一样，没有目录的概念，但是 key 的名称支持 /字符，从而实现起来像目录的伪目录，但是存储结构上不存在层级关系。

另外，为了提高存储性能，Kubernetes 在存储在 Etcd 中的 value是 protobuf 格式，我们不能直接查看其中的内容，需要借助 etcdhelper 工具来查看。查看命令如下：
...
解析 ReplicaSet 的资源定义之后，并可以根据其 Spec 定义进行资源调和，具体逻辑为：解析资源定义数据，例如：需要创建 Pod 的副本数为 2、需要创建的容器名和镜像地址、容器端口等。之后会调用 kube-apiserver 的 Pod 创建接口，来创建指定副本数的 Pod。创建的 Pod 名字格式为 my-replicaset-xxx，其中 xxx为 kube-apiserver 自定生成的字符串。
...
在 kube-scheduler Watch 到 Pod 的创建事件之后，会解析事件的对象数据，其实就是 Pod 的资源定义。并根据集群中的节点个数及状态，Pod 的资源定义，通过一系列的调度插件，将 Pod 调度到合适的节点上。在 kube-scheduler 找到一个适合 Pod 运行的 Node 之后，会将 NodeName 写入 Pod 的 spec.nodeName字段，例如：
...
kube-scheduler 通过将 节点名写入 Pod 的 spec.nodeName字段，来将 Pod 调度到该节点上。kube-sheduler 更新完 Pod 的资源定义后，会请求 kube-apiserver，将更新后的资源定义写入 Etcd。
...
kubelet 获取到 Pod 的资源变更之后，会过滤掉所有 spec.nodeName值不是 kubelet 所在节点的节点名的 Pod 资源的所有事件。这样，kubelet 就只会处理调度在其所在节点的 Pod，例如上述的 Pod。
...
Kubernetes调度Pod时有众多的调度算法可供选择。本小节，通过几个常用的调度示例，来给你展示下，具体如何在Kubernetes指定调度策略来调度Pod，并展示调度的结果。通过这些简单的调度示例，让你对Pod调度有个初步的了解。
...
在常见Pod时，我们可以根据需要同时指定多个调度策略，Kubernetes调度器会将Pod调度到所有满足这些调度策略的节点上。本小节，不会对调度策略进行过多说明，课程后面会详细介绍对应的调度策略。

### 指定资源请求和限制

在Kubernetes集群中创建Pod，通常都需要指定资源的请求（Request）和限制（Limit），这是最基础的调度策略。

我们先通过以下命令，来看下集群中每个节点的CPU和内存可分配量：

```shell
$ kubectl get nodes -o custom-columns='NODE:.metadata.name, CPU_ALLOCATABLE:.status.allocatable.cpu, MEMORY_ALLOCATABLE:.status.allocatable.memory'
NODE      CPU_ALLOCATABLE    MEMORY_ALLOCATABLE
k8s-01   16                 32557592Ki
k8s-02   12                 64458116Ki
```

可以看到 k8s-01节点可分配的CPU个数为16，k8s-02节点可分配的CPU个数为12。

下面是一个指定资源请求和限制的Pod资源定义，保存在 with-request-limit.yaml文件中，内容如下：
...
上面的Pod定义文件中，通过spec.schedulerName 指定由哪个Kubernetes调度器来调度该Pod。如果没有指定spec.schedulerName，Kubernetes在创建Pod时会设置默认的调度器为 default-scheduler。

Pod定义中CPU的资源请求量.spec.containers[0].resources.requests.cpu: "14"说明了Pod对CPU资源的请求量为 14。根据集群中节点的CPU可分配量，预期Pod会被调度到k8s-01节点上，因为k8s-02CPU可用量不满足Pod的CPU请求量。

通过以下命令在Kubernetes集群中创建Pod：

```shell
$ kubectl create -f with-request-limit.yaml # 创建 Pod
$ kubectl -n default get pods with-request-limit -owide
NAME                 READY   STATUS    RESTARTS   AGE   IP          NODE     NOMINATED NODE   READINESS GATES
with-request-limit   1/1     Running   0          3s    10.0.0.37   k8s-01   <none>           <none>
```

可以看到，Kuberentes默认的调度器将Pod成功调度到名为 k8s-01的节点上。

接下来，我们将 k8s-01 节点设置为不可调度，重新创还能Pod，来看下调度效果：

```shell
$ kubectl cordon k8s-01
node/k8s-01 cordoned
$ kubectl delete pods with-request-limit  --force
pod "with-request-limit" force deleted
$ kubectl create -f with-request-limit.yaml
pod/with-request-limit created
$ kubectl -n default get pods with-request-limit -owide
NAME                 READY   STATUS    RESTARTS   AGE   IP       NODE     NOMINATED NODE   READINESS GATES
with-request-limit   0/1     Pending   0          14s   <none>   <none>   <none>           <none>
```

可以看到 with-request-limit Pod 一直处在 Pending 状态。通过以下命令来看下Pod发生了什么：

```shell
$ kubectl -n default describe pods with-request-limit
...
Events:
  Type     Reason            Age                From               Message
  ----     ------            ----               ----               -------
  Warning  FailedScheduling  91s                default-scheduler  0/2 nodes are available: 1 Insufficient cpu, 1 node(s) were unschedulable. preemption: 0/2 nodes are available: 1 No preemption victims found for incoming pod, 1 Preemption is not helpful for scheduling.
  Warning  FailedScheduling  60s (x2 over 63s)  default-scheduler  0/2 nodes are available: 1 Insufficient cpu, 1 node(s) were unschedulable. preemption: 0/2 nodes are available: 1 No preemption victims found for incoming pod, 1 Preemption is not helpful for scheduling.
```

1 Insufficient cpu, 1 node(s) were unschedulable 说明了，集群中有1个节点处在 SchedulingDisabled状态，另外一个节点（k8s-02）因为不满足Pod的CPU资源请求量，而无法调度。

### 指定节点

在创建Pod时，我们还可以直接指定节点名字来将Pod调度到指定的节点上。这种方式在生产环境几乎不会使用，但是在测试环境或者测试的时候会被用到。

创建 Pod资源定义，并保存在 with-node-name.yaml 文件中，内容如下：
...
```shell
$ kubectl create -f with-node-name.yaml
pod/with-node-name created
$ kubectl get nodes
NAME     STATUS                     ROLES    AGE   VERSION
k8s-01   Ready,SchedulingDisabled   <none>   8d    v1.31.1
k8s-02   Ready                      <none>   8d    v1.31.1
$ kubectl -n default get pods with-node-name -owide
NAME             READY   STATUS    RESTARTS   AGE   IP          NODE     NOMINATED NODE   READINESS GATES
with-node-name   1/1     Running   0          21s   10.0.0.53   k8s-01   <none>           <none>
```

可以看到 Pod 被成功调度到了指定的 k8s-01 节点上。

这里，你可能会奇怪：k8s-01 已经被设置为不可调度，为什么 Pod还能被调度到k8s-01节点上？其实，Pod在创建时指定了.spec.nodeName，所以创建之后，对应节点的kubelet会直接在本节点创建该Pod，并没有通过调度器来给Pod分配节点。Kubernetes调度器只会调度.spec.nodeName 为空的Pod。

### 指定节点选择器

在企业创建Pod的过程中， 使用最多的调度方式是指定节点选择器来调度Pod。
